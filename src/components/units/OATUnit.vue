              ...
              <div v-for="metric in primaryMetrics" :key="`primary-${metric}`" class="bg-ion-light rounded-lg p-3 border-2 border-ion-primary">
                ...
                <div class="flex flex-col gap-1">
                  <div v-for="reading in getMetricReadings(metric)" :key="reading.deviceId"
                    ...
                    <span class="text-[0.8em] text-ion-medium">{{ reading.deviceType }} ({{ reading.deviceId.slice(-4) }})</span>
                  </div>
                </div>
              </div>
            </div>
...
const status = computed(() => getUnitStatus('OAT'));
const groupedSensors = computed(() => getGroupedSensors('OAT'));

// Get primary metrics for prominent display
const primaryMetrics = computed(() => getFilteredMetrics('OAT').primary);
