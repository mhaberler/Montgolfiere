    ...
    <div class="border-b border-gray-100 px-4 py-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <AppUnitIcon name="vario" class="text-gray-500" />
          <span>Vario</span>
        </div>
...
const status = computed(() => getUnitStatus("Vario"));
const groupedSensors = computed(() => getGroupedSensors("Vario"));
const filteredMetrics = computed(() => getFilteredMetrics("Vario"));
...
const formatMetricValue = (metric: string, value: any): string => {
  ...
  const formatters: Record<string, (v: any) => string> = {
    "distance (m)_m": (v) => `${v.toFixed(1)} m`,
    "speed_m/s": (v) => `${v.toFixed(1)} m/s`,
    "acceleration_m/s²": (v) => `${v.toFixed(1)} m/s²`,
  };
