      ...
      .map((runway) => {
        const surface =
          runway.surface?.mainComposite != null
            ...
            : ''
        const length = runway.dimension?.length ? `${runway.dimension.length.value} m` : ''
        const width = runway.dimension?.width ? `×${runway.dimension.width.value} m` : ''
...
export function isActive(
  ...
  const entry = hours.operatingHours.find((hour) => hour.dayOfWeek === todayDow)
  if (!entry) {
