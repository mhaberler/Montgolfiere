...
const locationToQnh = async (location: Position | null) => {
  ...
  try {
    ...
    const metarsWithDistance = validMetars.map((metar) => {
      ...
      const distanceKm = distance(userLocation, metarLocation, {
        units: "kilometers",
      });
    ...
    console.log(
      ...
      closestWithMetars.map(
        (m) => `${m.icaoId} (${m.distance}km, QNH: ${m.altim})`,
      ),
    );
...
const updateQnhFromLocation = async (
  ...
  try {
    ...
    if (closestWithMetars && closestWithMetars.length > 0) {
      ...
      console.warn(
        "No airports with METAR data found, keeping previous QNH value",
      );
...
watchEffect(() => {
  ...
  console.log(
    `currentQNH = ${currentQNH.value} currentQNHsource = ${currentQNHsource.value}.`,
  );
});
