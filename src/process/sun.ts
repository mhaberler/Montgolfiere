...
watch(
  [locationAvailable, elevationAvailable],
  (
    [newLocationAvailable, newElevationAvailable],
    [oldLocationAvailable, oldElevationAvailable],
  ) => {
    // Only trigger if availability status of either sensor changed
    const locationAvailabilityChanged =
      newLocationAvailable !== oldLocationAvailable;
    const elevationAvailabilityChanged =
      newElevationAvailable !== oldElevationAvailable;

    if (locationAvailabilityChanged || elevationAvailabilityChanged) {
      console.log(
        `Availability changed - Location: ${newLocationAvailable}, Elevation: ${newElevationAvailable}`,
      );
