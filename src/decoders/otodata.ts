...
// Define interface for Otodata sensor data
interface OtodataData {
  type?: "otodata";
  percent?: number;
  status?: number;
  name?: string;
...
export const parseOtodata = function (data: DataView): OtodataData {
  const otodata: OtodataData = { type: "otodata" };
  ...
  if (data.buffer.byteLength === 19) {
    otodata.percent = data.getUint16(9, true) / 100.0;
    otodata.status = data.getUint16(11, true);
  }

  if (data.buffer.byteLength === 22) {
    // LOG  {
    ...
    otodata.serial = data.getUint32(7, true).toString(10);
    otodata.model = data.getUint16(19, true).toString(10);
  }
