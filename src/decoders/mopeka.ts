...
// Define interface for Mopeka sensor data
...
export const parseMopeka = function (data: DataView): MopekaData {
  if (data.buffer.byteLength !== 10) {
    return {};
  }

  const mopeka: MopekaData = { type: "mopeka" };

  mopeka.batt = (data.getUint8(1) & 0x7f) / 32.0;
  mopeka.batpct = volt2percent(mopeka.batt);

  const tempAndSync = data.getUint8(2);
  mopeka.syncPressed = (tempAndSync & 0x80) > 0;
  mopeka.raw_temp = tempAndSync & 0x7f;
  mopeka.temperature = mopeka.raw_temp - 40; // °C
  mopeka.qualityStars = data.getUint8(4) >> 6;

  mopeka.accX = data.getUint8(8);
  mopeka.accY = data.getUint8(9);

  mopeka.raw_level = data.getUint16(3, true) & 0x3fff;
  mopeka.level =
    mopeka.raw_level *
    (MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_0 +
      MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_1 * mopeka.raw_temp +
      MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_2 *
        mopeka.raw_temp *
        mopeka.raw_temp);

  mopeka.level = Math.round(mopeka.level * 10) / 10;
  mopeka.macFrag = bytesToMacAddress(data, 5, 3);
