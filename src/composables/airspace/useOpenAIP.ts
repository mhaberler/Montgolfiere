...
export function useOpenAIP() {
  ...
  async function fetchAirspaceAt(lat: number, lng: number): Promise<AirspaceLookup> {
    ...
    try {
      ...
      const popupText = items.length
        ? items
            .map((airspace) => {
              const lower = airspace.lowerLimit ? formatAltitude(airspace.lowerLimit) : '?'
              const upper = airspace.upperLimit ? formatAltitude(airspace.upperLimit) : '?'
              ...
              const activity = airspace.activity ? ` – ${activityName(airspace.activity)}` : ''
          ...
          .map((airspace) => {
            ...
            return {
              ...
              properties: {
                ...
                lowerLabel: airspace.lowerLimit ? formatAltitude(airspace.lowerLimit) : '?',
                upperLabel: airspace.upperLimit ? formatAltitude(airspace.upperLimit) : '?',
  ...
  async function fetchAirportsAt(lat: number, lng: number): Promise<AirportItem[]> {
    ...
    try {
      ...
      const data = await response.json()
      return (data.items as AirportItem[]).filter((airport) => airport.frequencies?.length)
    } catch (error) {
