...
export class RunningRegression {
  ...
  push(x: number, y: number): void {
    ...
    this.Sxy +=
      ((this.xStats.mean() - x) * (this.yStats.mean() - y) * this.n) /
      (this.n + 1);
  ...
  correlation(): number {
    ...
    const denominator =
      this.xStats.standardDeviation() * this.yStats.standardDeviation();
  ...
  combine(other: RunningRegression): RunningRegression {
    ...
    combined.Sxy =
      this.Sxy + other.Sxy + (this.n * other.n * deltaX * deltaY) / combined.n;
