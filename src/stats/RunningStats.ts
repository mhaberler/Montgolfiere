...
export class RunningStats {
  ...
  private static readonly Z_VALUES = [1.645, 1.96, 2.576]; // 90%, 95%, 99%
  ...
  confidenceInterval(ci: ConfidenceInterval): number {
    ...
    if (ci < ConfidenceInterval.CI90 || ci > ConfidenceInterval.CI99)
      return NaN;

    return (
      (RunningStats.Z_VALUES[ci] * this.standardDeviation()) / Math.sqrt(this.n)
    );
  }
  ...
  combine(other: RunningStats): RunningStats {
    ...
    combined.M3 =
      this.M3 +
      other.M3 +
      (delta3 * this.n * other.n * (this.n - other.n)) /
        (combined.n * combined.n);
    combined.M3 +=
      (3.0 * delta * (this.n * other.M2 - other.n * this.M2)) / combined.n;
    ...
    combined.M4 +=
      (6.0 *
        delta2 *
        (this.n * this.n * other.M2 + other.n * other.n * this.M2)) /
        (combined.n * combined.n) +
